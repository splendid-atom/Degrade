...
public class EnemyBotAIpolygon_test : Enemy
{
    [Header("巡逻点设置")]
    ...
    [Header("视野可视化")]
    public bool showViewConeInGame = true;  // 是否在游戏中显示视野
    public Color viewConeColor = new Color(1, 0, 0, 0.2f);  // 视野锥颜色
    private GameObject viewConeObject;      // 视野锥游戏对象
    private LineRenderer viewConeRenderer;  // 视野锥线条渲染器

    [Header("攻击设置")]
    public float attackRange = 4f;        // 攻击范围
    public float hoverHeight = 3f;        // 悬停在玩家上方的高度
    public float horizontalOffset = 2f;   // 水平偏移量
    ...
    // 初始化视野锥
    private void InitializeViewCone()
    {
        // 创建视野锥游戏对象
        viewConeObject = new GameObject("ViewCone");
        viewConeObject.transform.parent = transform;
        viewConeObject.transform.localPosition = Vector3.zero;
        
        // 添加线条渲染器
        viewConeRenderer = viewConeObject.AddComponent<LineRenderer>();
        viewConeRenderer.useWorldSpace = false;
        viewConeRenderer.startWidth = 0.05f;
        viewConeRenderer.endWidth = 0.05f;
        viewConeRenderer.positionCount = 0;
        
        // 设置材质
        Material lineMaterial = new Material(Shader.Find("Sprites/Default"));
        viewConeRenderer.material = lineMaterial;
        viewConeRenderer.startColor = viewConeColor;
        viewConeRenderer.endColor = viewConeColor;
    ...
    {
        if (!showViewConeInGame || viewConeRenderer == null) return;
        
        // 设置视野锥的位置和方向
        Vector3 forward = (currentForwardDir.sqrMagnitude < 0.001f) ? Vector3.right : (Vector3)currentForwardDir;
        float halfAngle = viewAngle * 0.5f;
        
        // 计算视野锥的点
        int segments = 20;
        viewConeRenderer.positionCount = segments + 2;
        viewConeRenderer.SetPosition(0, Vector3.zero); // 中心点
        ...
        for (int i = 0; i <= segments; i++)
        {
            ...
            viewConeRenderer.SetPosition(i + 1, direction.normalized * viewRadius);
        }
    }
    
    // 设置视野锥的可见性
    private void SetViewConeVisibility(bool visible)
    {
        if (viewConeRenderer != null)
        {
            viewConeRenderer.enabled = visible;
        }
    }
    ...
    {
        // 清理视野锥资源
        if (viewConeObject != null)
        {
            Destroy(viewConeObject);
        }
    }
